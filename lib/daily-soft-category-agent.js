...
try {
  ...
  if (GOOGLE_AI_API_KEY && GOOGLE_AI_API_KEY !== "YOUR_GOOGLE_AI_KEY" && GoogleGenAI) {
    ...
    console.warn('⚠️ Google AI not configured - agent cannot run');
  }
} catch (err) {
  console.error('❌ Failed to load Google AI:', err.message);
}
...
{"selectedTerms": ["term1", "term2", "term3", "term4", "term5"]}`;
    ...
    console.log('🤖 Analyzing potential categories with LLM...');
    ...
    if (!result) {
      console.warn('⚠️ No response from LLM, falling back to simple scoring');
      return fallbackCategorySelection(potentialCategories, existingSoftCategories, maxTerms);
    }
