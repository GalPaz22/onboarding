...
// Optional dependencies (loaded dynamically if installed)
...
try {
  ...
  OpenAIEmbeddings = mod.OpenAIEmbeddings || (mod.default && mod.default.OpenAIEmbeddings) || null;
} catch {}
...
// How many products to run through Gemini/OpenAI concurrently. The previous
// strictly-sequential loop made a full catalog reprocess take hours; this
// bounds concurrent API calls the same way scraper/steimatzky.js does.
const REPROCESS_CONCURRENCY = Number(process.env.REPROCESS_CONCURRENCY) || 8;
...
const ai = GOOGLE_AI_API_KEY && GOOGLE_AI_API_KEY !== "YOUR_GOOGLE_AI_KEY" && GoogleGenAI
  ? new GoogleGenAI({ apiKey: GOOGLE_AI_API_KEY })
  : null;

// Initialize OpenAI embeddings if available
const embeddings = OPENAI_API_KEY && OPENAI_API_KEY !== "YOUR_OPENAI_API_KEY" && OpenAIEmbeddings
...
async function reprocessProducts({
  ...
  const isRunningFromCli = process.argv[1] && path.resolve(process.argv[1]) === path.resolve(__filename);
  ...
  // If incremental mode is active, only process soft categories
  if (incrementalMode) {
    console.log('🔄 INCREMENTAL MODE: Configuring for scoped soft category processing');
    console.log('   New soft categories:', incrementalSoftCategories);
    options = {
      ...options,
      reprocessHardCategories: false,  // Don't touch hard categories
      reprocessSoftCategories: true,   // Only update soft categories
      reprocessTypes: false,           // Don't touch types
      reprocessVariants: false,        // Don't touch variants
      reprocessEmbeddings: false,      // Don't regenerate embeddings
      reprocessDescriptions: false,    // Don't regenerate descriptions
      translateBeforeEmbedding: false  // No translation needed
    };
  }
  const logs = [];
  ...
  if (incrementalMode) {
    ...
    await log(`   ✅ Will ONLY update soft categories (preserving all other fields)`);
    await log(`   ✅ New soft categories being added: ${incrementalSoftCategories.join(', ')}`);
    await log(`   ✅ Total soft categories after merge: ${safeSoftCategories.length}`);
    await log(`   ✅ Scoped processing: Only products that can benefit from new categories\n`);
  }
  ...
  // Build query conditions based on sync mode and incremental mode
  let classificationConditions;

  if (incrementalMode) {
    // In incremental mode: target products that already have been classified
    // and could benefit from the new soft categories
    classificationConditions = {
      $and: [
        { category: { $exists: true, $ne: [], $ne: null } },  // Has categories
        { $or: [
          { softCategory: { $exists: false } },  // No soft category yet
          { softCategory: { $eq: [] } },         // Empty soft category
          { softCategory: { $ne: null } }        // Or has soft categories (to reclassify with new options)
        ]}
      ]
    };
    await log("🔄 Incremental mode: Will process IN-STOCK products that have been classified and can benefit from new soft categories");
  } else if (isImageSyncMode) {
    classificationConditions = {};
    await log("🖼️ Image mode: Will reprocess ALL IN-STOCK products with embeddings");
  } else {
    if (missingSoftCategoryOnly) {
      classificationConditions = {
        $and: [
      ...
      await log("🎨 Text mode: Will only process IN-STOCK products with categories but MISSING softCategory field");
    } else {
      classificationConditions = {
        $or: [
      ...
      await log("📝 Text mode: Will only process IN-STOCK products missing categories/types");
    }
  }
  ...
  if (options.reprocessEmbeddings) {
    // Covers three cases: already has an embedding (re-embed), has a
    // description1 but no embedding yet (embed it), or has neither (brand
    // new / never processed at all — e.g. a catalog synced by something
    // other than processWoo/processShopify, like a bespoke scraper).
    baseQuery = {
      $and: [
        inStockFilter,
        {
          $or: [
            ...
            { description1: { $exists: true, $ne: "" } },
            {
              $and: [
                ...
                { $or: [{ description1: { $exists: false } }, { description1: null }, { description1: "" }] }
              ]
            }
          ]
        }
  ...
  let finalQuery;
  if (incrementalMode) {
    // In incremental mode, always target products with embeddings (already processed)
    finalQuery = {
      $and: [
        inStockFilter,
        { embedding: { $exists: true, $ne: null } },
    ...
    await log("🔄 Incremental mode: Processing IN-STOCK products with embeddings for soft category updates");
  } else if (isImageSyncMode) {
  ...
  if (totalToProcess === 0) {
    if (incrementalMode) {
      await log("✅ Nothing to do. No products found that can benefit from the new soft categories.");
    } else if (isImageSyncMode) {
  ...
  const processOneProduct = async (product) => {
    try {
      ...
      // Lock file is gone — a stop request came in. Skip this product
      // rather than starting new API calls for it.
      return;
    }
    ...
    try {
      ...
      if (!originalDescription || originalDescription.trim() === "") {
        ...
        if (incrementalMode) {
          // In incremental mode, ONLY update soft categories and timestamp
          updateData = {
            categoryTypeProcessedAt: new Date()
          };
          await log(`🔄 Incremental mode: Will only update softCategory field for ${product.name}`);
        } else {
          // Normal mode: update all fields as usual
          updateData = {
            categoryTypeProcessedAt: new Date(),
  ...
  const limit = pLimit(REPROCESS_CONCURRENCY);
  await Promise.all(productsToProcess.map((product) => limit(() => processOneProduct(product))));
  ...
  if (incrementalMode) {
    await log("\n🎉 Incremental reprocessing complete!");
    await log(`   ✅ Updated soft categories for ${done} products`);
    await log(`   ✅ New soft categories added: ${incrementalSoftCategories.join(', ')}`);
    await log(`   ✅ All other product fields preserved (embeddings, descriptions, hard categories, types)`);
  }
