...
const { default: WooCommerceRestApi } = pkg;
 ...
 */
export async function classifyCategoryAndTypeWithGemini(enrichedText, productName, userCategories, userTypes, softCategories, variants = []) {
  try {
    // Check if Gemini AI is available
    if (!ai) {
 ...
 */
export async function processWooProducts({ wooUrl, wooKey, wooSecret, userEmail, categories, userTypes, softCategories, dbName }) {
  const logs = [];
  ...
  try {
    ...
    console.log(`✅ Created lock file: ${lockFilePath}`);
  } catch (err) {
    console.warn(`⚠️ Could not create lock file: ${err.message}`);
  }
  ...
  const shouldStop = async () => {
    try {
      ...
      return false; // Lock file exists, continue
    } catch {
      return true; // Lock file removed, stop
    }
  };
  ...
  try {
    ...
    // ============================================================================
    // SMART SYNC: Only process products WITHOUT embeddings (new products)
    // This allows incremental syncs - re-running won't reprocess existing products
    // ============================================================================
    
    // Check which products already have embeddings in the database
    const productIds = inStockProducts.map(prod => prod.id);
    await addLog(`\n🔍 Checking for existing embeddings in ${inStockProducts.length} products...`);
    await addLog(`   Query: { id: { $in: [...] }, embedding: { $exists: true, $ne: null } }`);
    ...
    await addLog(`\n📊 SYNC SUMMARY:`);
    await addLog(`   📦 Total products fetched: ${allProducts.length}`);
    await addLog(`   🏪 In-stock products: ${inStockProducts.length}`);
    await addLog(`   ✅ Already processed (have embeddings): ${existingProductIds.size}`);
    await addLog(`   🆕 New products (need processing): ${productsToProcess.length}`);
    await addLog(`   💡 Only NEW products will be enriched with AI\n`);
    ...
    if (productsToProcess.length === 0) {
      await addLog(`\n✨ All in-stock products already have embeddings!`);
      await addLog(`   This is a re-sync and no new products were found.`);
      await addLog(`   Basic data for all products was refreshed.`);
      await addLog(`   ℹ️  To reprocess existing products, use the REPROCESS endpoint instead.\n`);
      ...
      await addLog(`✅ WooCommerce sync complete for: ${userEmail}`);
      await addLog(`   📊 Products in database: ${inStockProducts.length}`);
      await addLog(`   📊 New products processed: 0`);
      await addLog(`   💡 Use /api/reprocess to update existing products`);
  ...
  return logs;
    ...
    for (const product of productsToProcess) {
      // Check for stop signal before processing each product
      if (await shouldStop()) {
        await addLog('🛑 Stop signal detected - halting processing gracefully');
        await statusCol.updateOne(
          { dbName },
          { $set: { state: "stopped", finishedAt: new Date() } }
        );
        await client.close();
        console.log('✅ MongoDB connection closed (stopped)');
        
        // Lock file was already removed by stop endpoint, but try just in case
        try {
          await fs.unlink(lockFilePath);
          console.log(`✅ Removed lock file: ${lockFilePath}`);
        } catch (err) {
          // Likely already removed by stop endpoint
        }
    ...
    await addLog(`\n✅ WooCommerce sync complete for: ${userEmail}`);
    await addLog(`   📦 Total products in database: ${inStockProducts.length}`);
    await addLog(`   🆕 New products enriched with AI: ${productsToProcess.length}`);
    await addLog(`   ⏭️  Products with existing embeddings: ${existingProductIds.size} (skipped)`);
    await addLog(`   💡 Next sync will only process NEW products\n`);
    await addLog("🔐 Closing MongoDB connection");
    ...
    try {
      await fs.unlink(lockFilePath);
      console.log(`✅ Removed lock file on error: ${lockFilePath}`);
    } catch (unlinkErr) {
      console.warn(`⚠️ Could not remove lock file: ${unlinkErr.message}`);
    }
